...
def hsja(randomimg,
    ...
    t1 = millis()
    # Initialize.
    InitialSuccess,perturbed = initialize(randomimg, sample, params,max_query)
    if not InitialSuccess:
        return TimeHistory,None
    ...
    for j in np.arange(num_iterations):
        ...
        gradf = approximate_gradient(randomimg, perturbed, num_evals,
                                     delta, params)
        ...
        if params['stepsize_search'] == 'geometric_progression':
            ...
            perturbed, dist_post_update = binary_search_batch(sample,
                                                              perturbed[None], randomimg, params)
            ...
            idx_perturbed = decision_function(randomimg, perturbeds, params)

            if np.sum(idx_perturbed) > 0:
                # Select the perturbation that yields the minimum distance # after binary search.
                perturbed, dist_post_update = binary_search_batch(sample,
                                                                  perturbeds[idx_perturbed], randomimg, params)
        ...
        t2 = millis()
        TimeHistory.append([randomimg.q, t2 - t1])
        if randomimg.q > max_query:
            break
    return TimeHistory, np.expand_dims(perturbed,0)

def decision_function(randomimg, images, params):
    """
    ...
    if images.shape[0]==1:
        ...
        return randomimg.decision(images)
    else:
        results=[]
        for i in range(images.shape[0]):
            images = clip_image(images, params['clip_min'], params['clip_max'])
            results.append(randomimg.decision(np.expand_dims(images[i],0)))
...
def approximate_gradient(randomimg, sample, num_evals, delta, params):
    ...
    # query the model.
    decisions = decision_function(randomimg, perturbed, params)
...
def binary_search_batch(original_image, perturbed_images, randomimg, params):
    ...
    while np.max((highs - lows) / thresholds) > 1:
        ...
        decisions = decision_function(randomimg, mid_images, params)
...
def initialize(randomimg, sample, params,max_query):
    """
    ...
    if params['target_image'] is None:
        ...
        while True:
            ...
            for i in range(5):#Set Initialization limitation to avoid infinit loop
                random_noise = random_noise*2
                success = decision_function(randomimg, random_noise[None], params)
            if success:
                break
            if randomimg.q>max_query:
                return success, None
        ...
        while high - low > 0.001:
            ...
            success = decision_function(randomimg, blended[None], params)
            if success:
                high = mid
            else:
                low = mid
...
def geometric_progression_for_stepsize(x, update, dist, randomimg, params):
    ...
    def phi(epsilon):
        ...
        success = decision_function(randomimg, new[None], params)
        return success
