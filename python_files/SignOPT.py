...
import torch
from python_files.Util import millis
...
class OPT_attack_sign_SGD(object):
    def __init__(self, randomimg, k=200, train_dataset=None):
    ...
    def attack_untargeted(self, alpha=0.2, beta=0.001, iterations=1000, query_limit=20000,
        ...
        for i in range(num_directions):
            ...
            if self.RandomImg.decision(clip_image(x0 + theta,0,1)):
                initial_lbd = LA.norm(theta)
                theta /= initial_lbd  # l2 normalize
                lbd, count = self.fine_grained_binary_search(theta, initial_lbd, g_theta)
                query_count += count
                if lbd < g_theta:
        ...
        print("==========> Found best distortion %.4f in %.4f seconds "
              "using %d queries: %d" % (g_theta, (t2-t1)/1000, query_count,self.RandomImg.q))
        ...
        for i in range(iterations):
            ...
            if svm == True:
                sign_gradient, grad_queries = self.sign_grad_svm(xg, initial_lbd=gg, h=beta)
            else:
                sign_gradient, grad_queries = self.sign_grad_v1(xg, initial_lbd=gg, h=beta)
            ...
            ## if all attemps dfailed, min_theta, min_g2 will be the current theta (i.e. not moving)
            ...
            if self.RandomImg.q > query_limit:
                break




        return TimeHistory,x0 + gg * xg

    def sign_grad_v1(self,theta, initial_lbd, h=0.001, D=4, target=None):
        """
        ...
        for iii in range(K):  # for each u
            ...
            if target is None and self.RandomImg.decision(x0 + initial_lbd * new_theta):  # success
                sign = -1
    ...
    ##########################################################################################
    def sign_grad_v2(self,theta, initial_lbd, h=0.001, K=200):
        """
        Evaluate the sign of gradient by formulat
        sign(g) = 1/Q [ \sum_{q=1}^Q sign( g(theta+h*u_i) - g(theta) )u_i$ ]
        """
        ...
        for _ in range(K):
            ...
            new_theta = theta + h * u
            new_theta /= LA.norm(new_theta)
            if not self.RandomImg.decision(x0 + initial_lbd * new_theta):
                ss = 1
            queries += 1
            sign_grad += sign(u) * ss
        sign_grad /= K
    ...
    def sign_grad_svm(self,theta, initial_lbd, h=0.001, K=100, lr=5.0, target=None):
        """
        Evaluate the sign of gradient by formulat
        sign(g) = 1/Q [ \sum_{q=1}^Q sign( g(theta+h*u_i) - g(theta) )u_i$ ]
        """
        ...
        for iii in range(K):
            ...
            if (target is None and
                    self.RandomImg.decision(
                        x0 + initial_lbd * new_theta)):
                sign = -1
    ...
    def fine_grained_binary_search_local(self,theta, initial_lbd=1.0, tol=1e-5):
        x0=self.RandomImg.img.numpy()
        nquery = 0
        lbd = initial_lbd

        # still inside boundary
        if not self.RandomImg.decision(x0 + lbd * theta):
            lbd_lo = lbd
            lbd_hi = lbd * 1.01
            nquery += 1
            while not self.RandomImg.decision(x0 + lbd_hi * theta):
                lbd_hi = lbd_hi * 1.01
                nquery += 1
                if lbd_hi > 20:
            ...
            lbd_lo = lbd * 0.99
            nquery += 1
            while self.RandomImg.decision(x0 + lbd_lo * theta):
                lbd_lo = lbd_lo * 0.99
                nquery += 1
        ...
        while (lbd_hi - lbd_lo) > tol:
            ...
            nquery += 1
            if self.RandomImg.decision(x0 + lbd_mid * theta):
                lbd_hi = lbd_mid
            else:
                lbd_lo = lbd_mid
    ...
    def fine_grained_binary_search(self,theta, initial_lbd, current_best):
        ...
        if initial_lbd > current_best:
            if not self.RandomImg.decision(x0 + current_best * theta):
                nquery += 1
                return float('inf'), nquery
            lbd = current_best
        else:
